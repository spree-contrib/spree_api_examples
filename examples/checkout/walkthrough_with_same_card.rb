    ...
    class WalkthroughWithSameCard
      def self.run(client)
        ...
        if response.status == 200
          ...
          File.write("payment.json", JSON.pretty_generate(order))
          payment = order["payments"].first {|p| p.state == "checkout"}
          gateway_customer_profile_id = payment["source"]["gateway_customer_profile_id"]
          gateway_payment_profile_id = payment["source"]["gateway_payment_profile_id"]
        else
...
Examples.run(Examples::Checkout::WalkthroughWithSameCard)
