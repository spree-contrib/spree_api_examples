    ...
    class CaptureOnDispatch
      def self.run(client)
        ...
        if response.status == 200
          ...
          order = JSON.parse(response.body)
          puts "Delivery posted"
          puts JSON.pretty_generate(order['payments'])
        ...
        if order['payments'].any? { |p| p['state'] == 'checkout' }
          client.failed "Payment is in invalid state: " + order['payments'][0]['state']
        end

        # Ship one shipment

        shipment = order['shipments'][0]
        response = client.put("/api/shipments/#{shipment['number']}/ship")
        if response.status == 200
          shipment = JSON.parse(response.body)
          puts JSON.pretty_generate(shipment)
          if order['state'] == 'shipped'
            client.succeeded "Shipment complete!"
          else
            client.failed "Shipment did not ship."
          end
        else
          client.failed "Shipment call failed."
        end
      end
    end
  end
